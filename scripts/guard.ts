...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    hint: "Передавайте учётные данные через менеджер секретов, а в задаче ссылайтесь на него.",
    // Значение нормализуем: кавычки и хвостовая пунктуация к секрету не относятся.
    accept: (m) =>
      !isPlaceholder(
        (m[1] ?? "")
          .replace(/["'`]/g, "")
